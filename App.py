...
import asyncio
import edge_tts  # <--- Ye line ab shamil hai
...
with st.sidebar:
    ...
    selected_lang = st.selectbox("🌍 Select Language", list(available_locales.keys()), index=0)
    ...
    if user_text.strip():
        with st.spinner(f"Generating {style_name} voice..."):
            try:
                ...
                if audio:
                    st.audio(audio)
                    st.download_button("📥 Download MP3", data=audio, file_name=f"khak_ai_{selected_v_label.split(' ')[0]}.mp3")
